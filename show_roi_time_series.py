...
def plot_player_profit_ratios(player_stats, num_instances, num_rounds_per_instance):
    plt.style.use('dark_background')
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.set_facecolor('#262626')  # Slightly darker grey background color

    # Plot each player's ROI as a line without labels
    for instance in range(1, num_instances + 1):
        data = player_stats[(player_stats['Instance'] == instance) & (
            player_stats['Round'] <= num_rounds_per_instance)]

        # Filter out None values (where total bets were zero)
